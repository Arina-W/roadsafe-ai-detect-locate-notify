...
if st.session_state.pred:
    ...
    if st.session_state.location_confirmed and st.session_state.manual_point:
        ...
        st.divider()
        st.subheader("📧 Send the report")
        # Prefer real email from Overpass; otherwise generate a plausible alias for demo visuals
        def make_fake_email(name: str) -> str:
            n = (name or "Ward Office").lower()
            # strip common suffixes
            n = re.sub(r"(city\s*office|ward\s*office|city\s*hall|town\s*hall|office|hall)$", "", n).strip()
            # remove JP kanji suffixes if name:en missing
            n = re.sub(r"(区役所|市役所|町役場|村役場|役場|出張所)$", "", n)
            # keep alnum only
            parts = re.findall(r"[a-z0-9]+", n)
            local = parts[0] if parts else "wardoffice"
            return f"{local}@wardoffice.jp"

        display_recipient = None
        if office and office.get("email"):
            display_recipient = office["email"]
        else:
            display_recipient = make_fake_email(office["name"] if office else "Ward Office")

        to_email_display = st.text_input("Recipient", value=display_recipient, disabled=True)
        DEMO_RECEIVER = "tokyoreportaddress@gmail.com"
